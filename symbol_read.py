		...
		if token_type_candidate is 'ki':
			if c in string.ascii_letters:
				...
				if c not in string.digits+'_' and to_keyword(charbuffer) in keywords:
					yield to_keyword(charbuffer), l, charbuffer[0][1]
					charbuffer = []
				else:
					yield from flush_characters(charbuffer, l)
					charbuffer = []
				if c.isspace():
			...
			if c in op_chars:
				...
				yield from yield_op(charbuffer, l)
				charbuffer = []
				if c.isspace():
					token_type_candidate = ''
...
def flush_characters(charbuffer,l):
...
if __name__ =='__main__':
	...
	with open(sys.argv[1]) as file:
		...
		prev_line_number = 1
		for symbol,line_number,col_number in symbol_generator:
